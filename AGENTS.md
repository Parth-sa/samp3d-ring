...
- **Custom model upload** (`loadCustomModel`): GLBs are run through `patchGlbWithDiamondMetadata` (toggle `#model-patch`) then `importSingle({path,file})`; `.3dm` loads via `Rhino3dmLoader2` with `setLibraryPath('https://cdn.jsdelivr.net/npm/rhino3dm@8.17.0/')` (needs internet for the wasm). Sets `usingCustomModel=true`; "Back to Catalog Builder" calls `buildRing()` which resets it. Invalid files throw "Invalid GLB file" and surface via `setError` — expected, not a bug.
- **Snapshot** (`downloadSnapshot`): `renderer.rendererObject.domElement.toBlob` → PNG download, after 2 rAF flushes.
- **Engraving renders as a 3D mesh on the band.** `updateEngraving3D()` draws the text to a `CanvasTexture` on a partial `TorusGeometry` parented to `getRingRoot()` (so it rotates with the ring). Placement is in `eng3d` (radiusScale/tube/arc/rotX/rotY), rotation order `YXZ` — `rotY` orbits the arc around the band axis, set to `-0.75` to sit at the bottom-inner surface (authentic inside-band engraving, read by tilting the ring). Tunable live via `window.__ringBuilder.eng3d` + `updateEngraving3D()`. Material uses real depth (band occludes it) — do NOT set `depthWrite:false`/high `renderOrder` or it floats through the band reversed.
- **Scene env brightness is `viewer.scene.envMapIntensity`, NOT `texture.intensity`.** Setting `.intensity` on the env texture object does nothing. Assigning `scene.envMapIntensity` auto-calls `refreshEnvMapIntensity()`, which updates `mat.envMapIntensity` on every material **except** those with `userData.separateEnvMapIntensity = true` (the patched diamonds) — so metal env brightness and diamond sparkle stay independent. `ring-builder.ts` → `applyMetalEnvSettings()`.
